...
### 2026-08-22 Compact browsers, splitter and shared JPEG Resample

* Работа начата с чистого локального baseline `66e4dbd` на ветке `diagnostic/lagging-layer-version-2026-08-21`; прежних пользовательских изменений в worktree не было. Коммит, push и PR в рамках этапа не выполнялись.
* Полностью выведен из production Halftone Collapse: удалены ID, реализация, controls, presets/random pools, previews и тестовые ожидания. Импорт старого проекта с `halftone-collapse-brush` безопасно мигрирует на Slice Displacement с предупреждением; упоминание Halftone в предыдущей секции — только исторический отчёт о состоянии на 2026-08-21.
* Добавлен общий `CompactIconBrowser`: категоризированная сетка Effect Picker, Mosh Add Effect и Image Brush Style, roving tab stop, стрелки/Home/End, Enter/Space, Escape, disabled items, NEW/META/LEGACY и доступные tooltip-описания. Effect preview закреплён и обновляется при hover/focus; legacy остаётся отдельным disclosure.
* Evolution recipe A/B и Gradient Start/End строятся из общего registry с фильтрацией по stage. JPEG Resample зарегистрирован один раз для Effect, Mosh и Image Brush; будущий совместимый registry item автоматически попадает в селекторы и тестируется отдельно.
* Между Controls и Layers добавлен доступный splitter: pointer capture, rAF обновление CSS variable, keyboard resize, double-click reset, localStorage, ResizeObserver и min-height clamp. Исправлена гонка первого mount при ещё пустом parent ref.
* Добавлен `jpeg-js 0.4.4` (BSD-3-Clause, 76 029 bytes unpacked, без зависимостей) и единое детерминированное JPEG encode/decode ядро. Ограничения: long edge ≤2048, passes ≤4, premultiplied RGB, byte-exact alpha restore, transparent RGB zero, seeded noise, sharpen и smooth/pixelated upscale.
* Codec отделён от main bundle: реальная JPEG-реализация подключена только в Brush/Mosh/Image Brush workers; production registry содержит metadata-only descriptor. Effect передаёт Worker только cropped dirty region, а selected-layer source собирается через `composeLayerPixelsRegion` без full-document allocation. Общий `countChangedPixels` вынесен в нейтральный utility.
* Mosh получил атомарное применение JPEG preset, per-pass progress и корректную rack-карточку. Image Brush использует существующий variant/cache pipeline на стадиях Tip / Per Stamp / Whole Trail; controls и evolution presets не обходят текущую архитектуру отмены и stale-job gate.
* Статичные JPEG Effect previews пересобраны. Production build: main 552.56 kB (gzip 168.51 kB), `jpeg-js` отсутствует в main и присутствует только в worker chunks; остаётся предупреждение Vite о превышении 550 kB на 2.56 kB.
* Edge acceptance: Effect short/long — full sync 0, fit 0, exact Undo/Redo, rAF ≥50 ms 0; Image Brush progressive — worker p50/p95/max 86.2/95.0/95.4 ms, commit 0.8/1.9/3.0 ms; Mosh 10 applies — latency 278.1/295.0/306.2 ms и max rAF gap 13.9 ms. Финальный Firefox 154 JPEG short (20 мазков): full sync 0, fit 0, history byte-exact, layer commit p50/p95/max 2/2/2 ms, upload 0/1/1 ms, rAF ≥50 ms 0.
* Финальная валидация: `npm run typecheck` ✓; `npm test -- --run` ✓ — 277/277 (17 files); `npm run build` ✓ — 1662 modules. Edge UI acceptance подтвердил 4 уникальных NEW, отсутствие Halftone, keyboard/focus preview, единый Mosh browser, 17 Image Brush styles и splitter persistence/clamp/reset. Main 552.56 kB / gzip 168.51 kB; codec присутствует только в worker chunks.

### 2026-08-22 Image Brush JPEG controls follow-up

* Из каждой Image Brush FX-карточки удалена дублирующая полоса Subtle / Medium / Strong / Broken / Extreme; точные Amount и Mix сохранены. Effect и Mosh amount presets не затронуты.
* Исправлена причина визуального «сломанного экрана» при Apply Noise / Apply Sharpen: невидимый absolute checkbox не был привязан к локальному label, и браузер при focus прокручивал весь overflow-hidden document примерно на 780 px. Toggle теперь имеет локальный containing block и реальный 20×11 px focus target поверх переключателя.
* High / Medium / Low вынесены в общую codec-free metadata-функцию; добавлен **Melt** для Effect, Mosh и Image Brush. После визуальной проверки нижний target ограничен 28 px во всех UI, randomization, Image Brush engine и центральной core-normalization; старые сохранённые 8 px также исполняются как 28 px, а источник меньше 28 px остаётся собственного размера. Melt сохраняет quality 1, 4 passes, RGB noise 0.32 и chroma bleed 0.65, но использует smooth upscale вместо грубой pixelated-мозаики. Image Brush Melt фиксирует Amount/Mix 100%; defaults и четырёхпроходный performance cap не изменены.
* Чистая in-app Browser проверка Noise/Sharpen: `scrollY=0`, `rootTop=0`, body 720 px, duplicate preset strip 0, console errors 0. Повторная JPEG floor-проверка фиксируется финальной валидацией ниже.
* Финальная JPEG floor acceptance: Melt показывает target/min 28 px, quality 1, smooth upscale, Amount/Mix 100%; `scrollY=0`, `rootTop=0`, console errors 0. Legacy target 8 на source ≥28 детерминированно равен target 28; source <28 остаётся собственного размера. Typecheck ✓; 280/280 тестов ✓; build ✓ (1663 modules, main 553.07 kB / gzip 168.70 kB, прежнее Vite size warning остаётся).

### 2026-08-22 Smooth Progressive Decay and stroke opacity fade

* Progressive Decay больше не меняет random seed между соседними key variants: структура повреждения остаётся узнаваемой, а сила растёт последовательно. Speed exponent сужен до мягкого диапазона, Exponential/Ease/Pulse/Random Walk сделаны менее скачкообразными; встроенный Progressive Decay теперь стартует с Linear.
